...
final def JNDINAME_CREATE_START_TOKEN = 'Command: create jndiname'
final def JNDINAME_CREATE_END_TOKEN = 'Command: commit'
...
if ((updateList != null && updateList.length() > 0) || (updateFile != null && updateFile.exists() && updateFile.size() > 0)) {
    ...
    def processLine = { String line ->
        if (line != null && line.trim().length() > 0) {
            println "------------------------------------------------------------"
            def jndinameInfo = (line.split('->', 2) as List).collectAll{it.trim()}
